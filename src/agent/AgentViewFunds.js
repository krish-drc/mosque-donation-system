...
export default function AgentViewFunds() {
  ...
  useEffect(() => {
    // 🔹 Get the logged-in agent info from sessionStorage
  ...
  // 🔹 Fetch only the agent's members
  const fetchAgentMembers = async () => {
    try {
      const membersQuery = query(collection(db, "members"), where("agentID", "==", agentId));
  ...
  // 🔹 Fetch only funds related to this agent’s members
  const fetchAgentFunds = async () => {
    try {
      const fundsData = await getDocs(collection(db, "funds"));
      const allFunds = fundsData.docs.map((doc) => ({ ...doc.data(), docId: doc.id }));
      // Filter only funds of this agent’s members
      const memberIds = members.map((m) => m.memberID);
      const filteredFunds = allFunds.filter((f) => memberIds.includes(f.memberId));
      setFunds(filteredFunds);
    } catch (err) {
      console.error("Error fetching agent funds:", err);
    }
  };
  ...
  return (
      ...
      <div className="mt-4">
        <h3 className="text-center mb-4">💼 My Members Fund Summary</h3>
        ...
        <Table striped bordered hover responsive>
          ...
          <tbody>
            ...
            <tr style={{ fontWeight: "bold", backgroundColor: "#f1f1f1" }}>
              ...
              <td>
                Rs {funds.filter((f) => f.type === "One-time").reduce((sum, f) => sum + f.amount, 0)}
              </td>
              <td>
                Rs {funds.filter((f) => f.type === "Monthly").reduce((sum, f) => sum + f.amount, 0)}
              </td>
              <td>
                Rs {funds.filter((f) => f.type === "Yearly").reduce((sum, f) => sum + f.amount, 0)}
              </td>
              <td>Rs {totalPaid}</td>
